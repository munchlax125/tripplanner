...
누르면 그 목록이 펼쳐지고, 다시 누르면 접힙니다. `move_groups`가 없으면 예전처럼 `flights`만 나열합니다.
...
`budget_source: 경로.yaml`을 넣고, 탭을 빼려면 그 파일을 없애면 됩니다.

일정에 날짜 블록이 없는 날(귀국일 등)의 지출은 **직전 나라에 합산**됩니다 — 나라별 합계가 총계와 어긋나지 않게.
