...
**넓은 화면(1024px 이상)은 grid 배치만 바꿉니다. DOM 순서는 폰 그대로 두세요.** 순서를 바꾸면 폰에서 지도 위치가 달라집니다.
개요는 `.hgrid` 안의 섹션을 `.hsec` 로 한 칸씩 감싸 왼쪽에 쌓고, 개요 지도(`.hs-map`)만 오른쪽에서 `grid-row:1 / -1` 로 끝까지 걸칩니다.
왼쪽 개수 `--hrows` 는 `build.py` 가 세어 넣고, 뒤에 붙는 `1fr` 줄이 지도 키를 받아 줍니다 —
이 줄이 없으면 지도가 키만큼 왼쪽 섹션 사이를 벌립니다. 개요에 섹션을 새로 넣으면 `hsec` 로 감싸고 `_hrows` 도 올리세요.
하루 카드의 지도는 넓은 화면에서도 **글 아래 카드 전체 폭**입니다 — 글 | 지도 두 칸으로 뒀더니 지도가 너무 작다고 해서 되돌렸습니다.
전부 `@media screen` 안이라 인쇄는 한 칸입니다.
**1024px 미만(폰·인쇄 포함)에서는 날짜 칸과 세로선을 없애고** 날짜를 카드 위 한 줄로 올려 카드를 꽉 채웁니다.
`view_css` 가 넣는 `.day-rail::before` 규칙(나라별 첫·마지막 날 선 다듬기)은 넓은 화면에서만 보입니다.
...
**변경을 마치면 묻지 말고 커밋하고 push 합니다.** 사용자가 폰에서 Pages 로 보기 때문에 push 전의 변경은 보이지 않습니다.
빌드나 검사가 실패했으면 push 하지 말고 알리세요. 소스를 고쳤으면 **빌드해서 생성물까지 함께 커밋**합니다 —
